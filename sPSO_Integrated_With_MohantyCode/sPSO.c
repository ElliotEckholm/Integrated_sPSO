...
struct result sPSO (size_t nDim, struct param param, struct problem pb, ptrfunction function, void *ffParams, struct psoParamStruct *psoParams)
{
	int d;
	double error;
	double errorPrev;
...
//struct psoParamStruct *psoParams = (struct psoParamStruct *) ;
...
switch(randCase)
{
	...
	for (s = 0; s < R.SW.S; s++)
	{
	  ...
	  R.SW.X[s].f = evaluateFitness(nDim, R.SW.X[s], function, ffParams, pb.SS,pb.objective);
